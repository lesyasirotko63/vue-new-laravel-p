    ...
    actions: {
        async newHandler({commit, dispatch}, payload) {
            ...
            try {
                ...
                dispatch('snackbar/showSnackbar', 'Products has been created', {root: true})
        ...
        async edit({commit, dispatch}, payload) {
            ...
            try {
                ...
                dispatch('snackbar/showSnackbar', 'Products has been updated', {root: true})
                commit(`getData`, result.data)
            } catch (e) {
                dispatch('snackbar/showSnackbar', e, {root: true})
            } finally {
        ...
        async searchCategories({commit, dispatch}, val) {
            try {
                if (val) {
                    const result = await axios(`/categories/autocomplete?query=${val}&limit=100`)
