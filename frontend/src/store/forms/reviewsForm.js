    ...
    actions: {
        async newHandler({commit, dispatch}, payload) {
            ...
            try {
                ...
                dispatch('snackbar/showSnackbar', 'Reviews has been created', {root: true})
        ...
        async edit({commit, dispatch}, payload) {
            commit('startLoading')
            try {
                const result = await axios.put(`/reviews/${payload.id}`, {id: payload.id, data: payload.data})

                dispatch('snackbar/showSnackbar', 'Reviews has been updated', {root: true})
                commit(`getData`, result.data)
            } catch (e) {
                dispatch('snackbar/showSnackbar', e, {root: true})
            } finally {
