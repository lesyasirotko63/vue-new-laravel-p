  ...
  actions: {
    async newHandler({ commit, dispatch }, payload) {
      ...
      try {
        ...
        dispatch('snackbar/showSnackbar', 'Promo_codes has been created', {
          root: true,
        });
    ...
    async edit({ commit, dispatch }, payload) {
      commit('startLoading');
      try {
        const result = await axios.put(`/promo_codes/${payload.id}`, {
          id: payload.id,
          data: payload.data,
        });

        dispatch('snackbar/showSnackbar', 'Promo_codes has been updated', {
          root: true,
        });
        commit(`getData`, result.data);
      } catch (e) {
