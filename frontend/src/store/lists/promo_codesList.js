...
export default {
    ...
    actions: {
        async getData({ commit, dispatch }, { page, limit, field, sort }) {
            commit('showLoader')
            try {
                const response = await axios.get(`/promo_codes`, { params: { page, limit, field, sort } })
        ...
        async getFilteredData({commit}, request) {
            try {
                commit('showLoader')
                const response = await axios.get(`/promo_codes${request}`)
                commit('getData', response.data.rows)
                commit('hideLoader')
            } catch (e) {
        ...
        async deleteItem({dispatch, state}) {
            try {
                await axios.delete(`/promo_codes/${state.deleteId}`)
                dispatch('snackbar/showSnackbar', 'Promo_codes has been deleted', {root: true})
                dispatch('getData')
            } catch (e) {
                dispatch('snackbar/showSnackbar', e, {root: true})
            }
