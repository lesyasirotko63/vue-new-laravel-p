...
export default {
    ...
    actions: {
        async getData({ commit, dispatch }, { page, limit, field, sort }) {
            commit('showLoader')
            try {
                const response = await axios.get(`/products`, { params: { page, limit, field, sort } })
        ...
        async deleteItem({dispatch, state}) {
            try {
                await axios.delete(`/products/${state.deleteId}`)
                dispatch('snackbar/showSnackbar', 'Products has been deleted', {root: true})
                dispatch('getData')
            } catch (e) {
                dispatch('snackbar/showSnackbar', e, {root: true})
            }
