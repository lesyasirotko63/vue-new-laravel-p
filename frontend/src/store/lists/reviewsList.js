...
export default {
    ...
    actions: {
        async getData({ commit, dispatch }, { page, limit, field, sort }) {
            commit('showLoader')
            try {
                const response = await axios.get(`/reviews`, { params: { page, limit, field, sort } })
        ...
        async getFilteredData({commit}, request) {
            try {
                commit('showLoader')
                const response = await axios.get(`/reviews${request}`)
                commit('getData', response.data.rows)
                commit('hideLoader')
            } catch (e) {
        ...
        async deleteItem({dispatch, state}) {
            try {
                await axios.delete(`/reviews/${state.deleteId}`)
                dispatch('snackbar/showSnackbar', 'Reviews has been deleted', {root: true})
                dispatch('getData')
            } catch (e) {
                dispatch('snackbar/showSnackbar', e, {root: true})
            }
