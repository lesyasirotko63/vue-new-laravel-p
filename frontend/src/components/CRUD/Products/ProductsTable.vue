      ...
      >
            <template v-slot:top>
              ...
              ></v-text-field>
               ...
               <template v-slot:item.image="{ item }">
                  <div class="pa-1">
                    <div
                      :style="{background: `url(${dataFormatter.oneImageFormatter(item.image)}) center center / cover`}"
                      class="table-img"
                    ></div>
                  </div>
               </template>
              ...
              <template v-slot:item.categories="{ item }">
                <span
                  v-for="(i, idx) in dataFormatter.categoriesManyListFormatter(item.categories)"
                  :key="idx + item.categories"
                  class="d-block"
                 >
                    {{ i }}
                </span>
              ...
              <template v-slot:item.moreProducts="{ item }">
                <span
                  v-for="(i, idx) in dataFormatter.productsManyListFormatter(item.moreProducts)"
                  :key="idx + item.moreProducts"
                  class="d-block"
                 >
                    {{ i }}
                </span>
        ...
        <template v-slot:activator="{ on, attrs }">
            ...
            >
              ...
              <v-list>
                <router-link class="text-decoration-none" :to="`/admin/products/${item.id}/edit`">
                  <v-list-item>
...
export default {
  ...
  data() {
    return {
      ...
      showFilters: false,
      filters: [{label: 'Title', title: 'title'},{label: 'Description', title: 'description'},
      {label: 'Rating', title: 'rating', number: 'true'},
      {label: 'Price', title: 'price', number: 'true'},{label: 'Discount', title: 'discount', number: 'true'},
      ],
        options: {},
    }
  },
  ...
  computed: {
    ...mapState({
      ...
      tableData: state => state.products.rows,
      modalOpen: state => state.products.modalOpen,
      deleteId: state => state.products.deleteId,
    ...
    externalLink: () => {
      return process.env.NODE_ENV === 'production'
        ? window.location.origin + '/api-docs/#/Products'
  ...
  methods: {
    ...mapActions({
      ...
      getFilteredData: 'products/getFilteredData',
      deleteItem: 'products/deleteItem',
    }),
    ...mapMutations({
      setDeleteId: 'products/setDeleteId'
    }),
    delFilter(index) {
    ...
    rowClickHandler(row) {
      this.$router.push(`/admin/products/${row.id}/edit`)
    }
  },
}
</script>
