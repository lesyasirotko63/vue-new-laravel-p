<template>
    <div>
      ...
      <div class="my-4 ml-3">
        <a
          :href="externalLink">
          API documentation for promo_codes
        </a>
      </div>
      ...
      >
            <template v-slot:top>
              ...
              <template v-slot:item.products="{ item }">
                <span
                  v-for="(i, idx) in dataFormatter.productsManyListFormatter(item.products)"
                  :key="idx + item.products"
                  class="d-block"
                 >
                    {{ i }}
                </span>
        ...
        <template v-slot:activator="{ on, attrs }">
            ...
            >
              ...
              <v-list>
                <router-link class="text-decoration-none" :to="`/admin/promo_codes/${item.id}/edit`">
                  <v-list-item>
...
export default {
  ...
  computed: {
    ...mapState({
      loading: state => state.promo_codes.loading,
      tableData: state => state.promo_codes.rows,
      modalOpen: state => state.promo_codes.modalOpen,
      deleteId: state => state.promo_codes.deleteId,
      count: state => state.promo_codes.count,
    }),
    dataFormatter() {
  ...
  methods: {
    ...mapActions({
      getData: 'promo_codes/getData',
      getFilteredData: 'promo_codes/getFilteredData',
      deleteItem: 'promo_codes/deleteItem',
    }),
    ...mapMutations({
      setDeleteId: 'promo_codes/setDeleteId'
    }),
    delFilter(index) {
    ...
    rowClickHandler(row) {
      this.$router.push(`/admin/promo_codes/${row.id}/edit`)
    }
  },
}
</script>
